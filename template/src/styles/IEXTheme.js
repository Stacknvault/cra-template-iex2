import { ffmap } from '../lib/Context'
const getBaseTheme = () => {
    const accent = ffmap`legacyExposeSettings.colors.accent`;
    const accentContrast = ffmap`legacyExposeSettings.colors.accentContrast`;

    return {
        brand: {
            // colors: {
            //   color1: '#7FB069', //lock
            //   color2: '#E6F8DE', //lock
            //   color3: '#12100E',
            //   color4: '#34312D',
            //   color5: '#048BA8', //lock
            // },
            // colors: {
            //     primary: '#182825', // primary brand color
            //     secondary: '#225560', //secondary brand color
            //     background: '#FFFAFF', // background on pages
                

            //     baseText: '#303036', // default text color
            //     primaryText: '#FFFAFF', // color of text on primary
            //     secondaryText: '#E6E8E6', // color of text on secondary

            //     alternateRow: 'rgba(183,109,104,0.3)', // color of alternate row

                
            // },
            
            colors: {
                primary: accent||'#3E5641', // primary brand color
                secondary: accentContrast || '#A24936', //secondary brand color
                background: '#888', // background on pages
                

                baseText: '#FFF', // default text color
                primaryText: '#FFFAFF', // color of text on primary
                secondaryText: '#E6E8E6', // color of text on secondary

                alternateRow: 'rgba(255,255,255,0.3)', // color of alternate row

                
            },
        }
    }
};

export { getBaseTheme };